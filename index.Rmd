...
# a valid token
token <- "CAACEdEose0cBAEti9g4OOMHsKz3lbcZA8QEB0wOpxWq3OLZC3pI7HFKUQrOZAXMi2pENeyBP6VZBZAEukJSJiASaGOaB24pA3zYpwZAxtBPWNhPsZCXfmmkmN3FFKdcwrxgG9piZAKZBmtyievARFlsxGjBiepumtTUZCiYvSs1ZCK36QSRbvRbJ7TI2O3PnausI4DF2J7UpWRdcAZDZD"
...
```{r plot: ggplot2, warning=FALSE}
like_trending1 %>% 
    ggplot(aes(x=timed, y=n_like)) + geom_line() + geom_point()

like_trending2[name %in% top_likes$name[1:10]] %>%
...
3. OBSERVE the `posts` edge of your friends
    ...
    + possibly due to some un-documented API features
...
+ why use it?
    ...
    + developed by [Hadley Wickham](https://github.com/hadley/stringr)
        + who also writes `httr`, and many others
...
(gossip_titles <- gossip %>% 
    ...
    html_text())
grep("[公告]", gossip_titles, value=TRUE)

# now make a comprehensive function for multi-page crawling
crawlGossip <- function(npage, pat) {
    ...
    oneshot <- function(page_url, ...) {
        require(rvest)
        require(httr)
        gossip <- GET(page_url,
                  set_cookies(over18=1))$content %>% 
            read_html %>% 
            html_nodes("div .title") %>% 
            html_text() %>%
            gsub("\n\t*", '', .) %>%
    ...
    res <- parLapplyLB(cl, all_pages, oneshot)
    res[sapply(res, function(x) length(x) > 0)] %>% 
        unlist
}

wanted <- crawlGossip(100, "^\\[問卦\\] 有沒有.*的八卦？")
wanted
```
...
library(wordcloud)
buzz <- str_match(wanted, "有沒有(.*)的八卦？")[,2] %T>% 
    {
        par(family='Heiti TC Light')
        wordcloud(., min.freq=1, rot.per=.5, scale=c(1,2), 
