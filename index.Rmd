...
```{r dcard content}
crawlDcardArticles <- function(target="bg", npage, title_pat=NULL, article_pat) {
    ...
    crawlSingleArticle <- function(page_url, pat) {
        require(rvest)
        require(httr)
        response <- GET(page_url)
        if ( (rcode <- response$status_code) != 200 ) {
            ...
            content(response)$version[[1]]$content %>%
                grep(pat, ., value=TRUE)
        }
    }
    ...
    parLapplyLB(cl, pages, crawlSinglePageTitleId, pat=title_pat) %>% 
        ...
        parLapplyLB(cl, ., crawlSingleArticle, pat=article_pat) %>% 
        unlist
}

system.time(
    dcardarticles <- crawlDcardArticles("bg", 10, article_pat="室友")    
)
